...
<body>
    <h3>변수 선언 및 할당</h3>
    <!-- 
    EL 및 JSTL은 영역에 저장된 속성을 이용해서 연산하거나 출력함.
    따라서 스크립트릿에서 선언한 변수는 EL에서 사용할 수 없음.
    ...
    <!-- 
    Tomcat8.0 부터는 EL에서 변수할당을 할 수 있다. 하지만 할당과 동시에 출력되므로
    ...
    <!-- 
    나눗셈에서는 /, % 대신에 div, mod 를 사용.
    또한 자동형변환이 적용되므로 정수끼리의 연산에서 실수의 결과를 반환할 수 있다.
     -->
   ...
   문자열의 연결로 사용은 불가능.
    -->
    <h3>+ 연산자는 덧셈만 가능</h3>  
    <!-- 문자열 34가 자동으로 정수로 변경되어 연산됨. Integer.parseInt()를 
    적용한것과 동일한 결과가 반환됨  -->
    num1 + "34" : ${ num1 + "34" } <br />
    <!-- 핮만 숫자와 문자열 혹은 문자열과 문자열의 경우에는 덧셈 연산이 
    불가능하므로 예외가 발생한다 -->
    num2 + "이십" : \${num2 + "이십" }  <br /> 
    "삼십" + "사십" : \${"삼십" + "사십" } 
	...
	<!-- 
	논리 And, 논리 Or 사용시 && / || 대신 영문자 형식의 연산자 사용가능
	 -->
    <h3>논리 연산자</h3>
