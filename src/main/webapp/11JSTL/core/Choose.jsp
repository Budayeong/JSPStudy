...
choose태그
 ...
 -->
	...
	<!-- choose태그와 when태그 사이에 HTML주석을 기술하면 에러가 발생.
	...
	<c:choose>
		...
		<c:otherwise>
			<!-- else와 동일한 역할. 위 조건에 만족하지 않을때 실행되는 블럭  -->
			${ number }는 홀수입니다.
		</c:otherwise>
	</c:choose>
	...
	<c:if test="${ not (empty param.kor or empty param.eng or empty param.math) }">
		<!-- 3과목의 점수를 더한 후 평균을 계산. 이 값은 avg에 저장됨
		EL은 자동으로 형변환되므로 경우에 따라 실수의 결과가 나올 수 있다 -->
		<c:set var="avg" value="${(param.kor + param.eng + param.math)/3}"/>
		평균 점수는 ${ avg }으로
		
		<!-- 
		평균점수를 통해 조건을 걸어 학점을 출력. when 태그는 if/ if else와 
		동일하고, otherwise는 else 와 동일하다
		 -->
		<c:choose>
