...
<body>
	...
	forTokens태그: 문자열에서 특정 구분자를 통해 반환된 배열의 크기만큼
		반복해야할때 사용
		속성] 
			itmes : 구분자를 포함한 문자열을 지정. 단 컬렉션이나 배열은 사용할 수 없음
			delims(delimiter) : 구분자를 지정. 여러가지 특수기호를 사용 할 수 있음
			var : 구분자를 통해 잘라낸 토큰을 저장
			* 토큰이란? : 문법적으로 의미있는 최소단위. 하이픈으로
			구분되어있는 전화번호를 분리했을때 각각의 번호가 토큰이 됨
	 -->
	<%
// 	, 로 구분된 문자열 정의
