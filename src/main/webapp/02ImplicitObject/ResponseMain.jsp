...
<body>
    <h2>1. 로그인 폼</h2>
    <%
// 	get방식으로 전달된 loginErr 파라미터가 있는 경우(값없이 loginErr이 전달만돼도)에만 "로그인실패" 출력
// 	첫 실행시에는 파라미터가 없는 상태이므로 메시지는 출력되지 않음
    String loginErr = request.getParameter("loginErr");
    if (loginErr != null) out.print("로그인 실패");
    %>
    <!--
    로그인을 위해 post방식으로 폼값을 전송함. get 방식으로 전송하면
    로그인 정보가 쿼리스트링을 통해 주소줄에 남기때문에 정보유출의 위험이 있음
    로그인과 같이 보안이 필요한 경우에는 post방식을 사용해야함
      -->
    ...
    +09 즉 9시간이 빠르므로 그 만큼을 더해줘야 정상적인 날짜가 출력됨
